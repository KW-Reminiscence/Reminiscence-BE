...
def test_public_demo_replaces_an_abandoned_active_session(tmp_path: Path) -> None:
    client, activity_path, _ = client_with(tmp_path)
    abandoned_session_id = start_session(client)
    ...
    assert started.status_code == 201
    replacement_session_id = started.json()["session_id"]
    assert replacement_session_id != abandoned_session_id

    sessions = {
        session["session_id"]: session
        for session in json.loads(activity_path.read_text(encoding="utf-8"))[
            "conversation_sessions"
        ]
    }
    assert sessions[abandoned_session_id]["status"] == "COMPLETED"
    assert sessions[abandoned_session_id]["completion_reason"] == "NAVIGATION"
    assert sessions[replacement_session_id]["status"] == "ACTIVE"
