...
| `flow.py` | 대화 아크(진입→심화→확장→마무리) 상태 기계. 대화를 어떻게 이어갈지. |
| `themes.py` | 회상 절정 시기의 생애 주제와 감각 단서. 무엇을 실마리로 삼을지. |
...
### 대화를 어떻게 이어가는가 (대화 아크 × 생애 주제)

시나리오(S1~S6)가 "무엇에 대해 말하는가"라면, 회상 시나리오(S1~S2)는 그 위에
두 축을 더 얹습니다 — "지금 대화의 어느 지점인가"(단계)와 "무엇을 실마리로
삼는가"(생애 주제).

```
진입 ──► 심화 ──► 확장 ──► 마무리
opening  deepening broadening wrapping
```

- **진입**: 사진을 따뜻하게 짚고 부담 없는 열린 질문 하나.
- **심화**: 어르신이 반응하면 감각(냄새·맛·소리)·사람·감정으로 한 걸음 더.
- **확장**: 충분히 이야기했으면 생애 흐름을 따라 다음 주제로 넓힘.
- **마무리**: 반응이 잦아들면 새 질문 없이 따뜻하게 정리.

단계 전이는 어르신의 직전 발화가 얼마나 적극적이었는지(적극/짧음/혼란)로
규칙에 따라 결정합니다(LLM에 다시 묻지 않아 지연이 없습니다). 짧은 반응이
이어지면 부드럽게 마무리로, 마무리에서 다시 말문을 여시면 심화로 되돌립니다.

**생애 주제**(`themes.py`)는 회상요법 문헌에 근거합니다.

- *회상 절정(reminiscence bump)*: 치매에서도 아동기~초기 성인기(약 6~30세)
  기억은 상대적으로 보존됩니다. 주제를 이 시기(어린 시절 동네 → 학창 시절 →
  젊은 날의 일 → 결혼 → 육아)에 둡니다.
- *문화적 생애 각본*: 위 삶의 순서 자체가 기억 인출의 길잡이가 되므로, 확장
  단계에서 다음 주제로 넘어갈 때 이 순서를 따릅니다.
- *사람 중심(SolCos)*: 주제를 기기가 정해 끌고 가지 않습니다. 어르신 발화에서
  주제를 감지해 따라가고, 그 주제의 감각 단서(아궁이 불 냄새, 풍금 소리,
  도시락 반찬 냄새…)를 심화 질문의 재료로 씁니다.
- *감각 단서 우선*: "언제였어요?"류 사실 질문 대신 냄새·맛·소리·촉감을 묻습니다.
- *창작 전환(TimeSlips)*: 기억이 안 난다고 하시면 "괜찮아요"로 물러선 뒤 기억을
  상상으로 바꿔 묻습니다("어땠을 것 같으세요?"). 정답이 없으니 실패도 없습니다.

각 단계·주제에 맞는 지침이 프롬프트에 덧붙어, 매 턴 같은 질문을 반복하지 않고
대화가 자연스럽게 깊어지고 넓어집니다. 턴 응답의 `phase` 필드로 현재 단계를
확인할 수 있습니다.

주제 목록은 1940~60년대에 유년기를 보낸 세대 기준의 데이터이며, 사용성
조사(복지관·요양원 인터뷰) 결과에 따라 `themes.py`만 고치면 됩니다.
...
| `PATCH` | `/dialogue/sessions/{id}` | 표시 중인 사진·루틴 변경 |
