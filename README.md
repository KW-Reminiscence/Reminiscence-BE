...
시나리오(S1~S6)가 "무엇에 대해 말하는가"라면, 회상 시나리오(S1~S3)는 그 위에
"지금 대화의 어느 지점인가"를 나타내는 단계를 하나 더 얹습니다. 회상요법의
대화 흐름을 그대로 옮긴 것입니다.
...
- **진입**: 사진·음악을 따뜻하게 짚고 부담 없는 열린 질문 하나.
- **심화**: 어르신이 반응하면 감각(소리·냄새·맛)·사람·감정으로 한 걸음 더.
- **확장**: 충분히 이야기했으면 이어지는 다른 기억으로 넓힘.
...
주제(사진·음악)가 바뀌면 아크는 진입부터 다시 시작합니다.

각 단계에 맞는 이어가기 지침이 프롬프트에 덧붙어, 매 턴 같은 질문을 반복하지
않고 대화가 자연스럽게 깊어지고 넓어집니다. 턴 응답의 `phase` 필드로 현재
단계를 확인할 수 있습니다.
